...
# --- 2. HÀM TIỆN ÍCH VỀ THỜI GIAN (TÍCH HỢP TỪ FILE CỦA BẠN) ---
...
def parse_natural_time(expression: str, base_date: datetime) -> tuple[datetime, datetime]:
    ...
    # Logic ưu tiên: Bắt các cụm có số lượng trước
    ...
    elif "tháng trước" in expr:
        new_start = add_months(new_start, -1)
    elif "năm sau" in expr:
        new_start = add_years(new_start, 1)
    elif "năm trước" in expr:
        new_start = add_years(new_start, -1)
    elif "đầu tháng" in expr:
        new_start = new_start.replace(day=1)
    elif "giữa tháng" in expr:
        new_start = new_start.replace(day=15)
    elif "cuối tháng" in expr:
        new_start = end_of_month(new_start)
    elif "đầu năm" in expr:
        new_start = new_start.replace(month=1, day=1)
    elif "giữa năm" in expr:
        new_start = new_start.replace(month=6, day=15)
    elif "cuối năm" in expr:
        new_start = new_start.replace(month=12, day=31)

    # Mặc định sự kiện dài 1 tiếng
...
def add_months(dt: datetime, months: int) -> datetime:
    ...
    month = month % 12 + 1
    day = min(dt.day, 28)  # Giới hạn ngày để tránh lỗi tháng ngắn
...
def tao_task_va_len_lich(tieu_de: str, thoi_gian_bat_dau: str, thoi_gian_ket_thuc: str) -> str:
    ...
    # (Giữ nguyên code, không thay đổi)
    try:
...
def tim_lich_trinh(ngay_bat_dau: str, ngay_ket_thuc: str) -> str:
    ...
    try:
        with engine.connect() as connection:
            ...
            return f"🔎 Có {len(events)} sự kiện: {'; '.join(events)}"
    except Exception as e:
...
def chinh_sua_task(tieu_de_cu: str, thoi_gian_moi: str) -> str:
    """Chỉnh sửa thời gian của một sự kiện đã có, có thể nhận thời gian tuyệt đối (vd: '15h ngày mai') hoặc tương đối (vd: '2 tuần sau')."""
    try:
        with engine.connect() as connection:
            with connection.begin() as transaction:
                find_query = text(
                ...
                # Gọi hàm parse_natural_time để tính toán thời gian mới
...
QUY TẮC:
1. Tự tính toán ngày tháng và truyền vào công cụ:
...
2. Với yêu cầu CHỈNH SỬA, hãy trích xuất toàn bộ cụm thời gian mới (vd: '2 tuần sau', 'đầu tháng tới') và truyền vào tham số `thoi_gian_moi` của công cụ `chinh_sua_task`.
3. Nếu không có giờ cụ thể -> mặc định 09:00 - 10:00. Nếu có giờ bắt đầu, không có giờ kết thúc -> mặc định kéo dài 1 tiếng.
"""
...
app = FastAPI(title="Skedule AI Agent API", version="1.2.0")
